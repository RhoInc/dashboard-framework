(function (global, factory) {
    ...
    function data$1(specification) {
      ...
      if (!(typeof specification.data === 'string' || Array.isArray(specification.data))) {
        console.warn('[ data ] property of chart specification must be an array or a path to a .csv file.');
        specification.continue = false;
        return;
      }
    }
    ...
    function setTitle(chart) {
      ...
      chart.containers.settingsToggle = chart.containers.title.append('span').classed('df-settings-toggle df-invisible', true).html(' &#x2699;').attr('title', 'View chart settings.');
    ...
    function withSchema(chart) {
      ...
      chart.controlInputs = chart.configuration.syncControlInputs(chart.controlInputs, chart.settings); //Check schema against specified data mappings.
    ...
    function addWarning(chart) {
      ...
      chart.containers.warningRequired = chart.containers.dataCheck.append('strong').classed('df-data-check__warning df-data-check__warning--required', true);
      chart.containers.warningOptional = chart.containers.dataCheck.append('em').classed('df-data-check__warning df-data-check__warning--optional', true);
    }
    ...
    function updateWarning(chart) {
      ...
      chart.containers.warning.classed('df-hidden', missingVariables.length > 0).text('All variables present.');
      chart.containers.warningRequired.classed('df-data-check__warning--missing', missingRequiredVariables.length > 0).classed('df-hidden', missingVariables.length === 0).text(missingRequiredVariables.length > 0 ? "".concat(missingRequiredVariables.length, " required variable").concat(missingRequiredVariables.length === 1 ? '' : 's', " missing.") : "All required variables present.");
      chart.containers.warningOptional.classed('df-data-check__warning--missing', missingOptionalVariables.length > 0).classed('df-hidden', missingVariables.length === 0).text(missingOptionalVariables.length > 0 ? "".concat(missingOptionalVariables.length, " optional variable").concat(missingOptionalVariables.length === 1 ? '' : 's', " missing.") : "All optional variables present.");
    }
      ...
      chart.containers.schemaDropdowns.on('change', function (d) {
        ...
        if (option !== 'None selected') {
          ...
          if (chart.configuration) {
            ...
            chart.settings = chart.configuration.syncSettings(chart.settings);
            chart.controlInputs = chart.configuration.syncControlInputs(chart.controlInputs, chart.settings);
          } else {
    ...
    function initializeChart(chart) {
      ...
      if (typeof chart.data === 'string') {
        d3$1.csv(chart.data, function (d) {
          ...
          if (chart.variables.schema.filter(function (variable) {
            return variable.required && variable.missing;
          }).length === 0) {
            chart.containers.dataCheck.classed('df-hidden', true);
